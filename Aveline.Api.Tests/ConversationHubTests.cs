...
public class ConversationHubTests
{
    ...
    [Fact]
    public async Task JoinSalon_ActiveMember_JoinsSalonGroup()
    {
        var userId = Guid.NewGuid();
        var orgId = Guid.NewGuid();
        var conversationId = Guid.NewGuid();
        var clerkId = "conv_hub_join";
        var user = MakeUser(userId, clerkId);
        var memberships = new List<OrganizationMembership>
        {
            new() { OrganizationId = orgId, UserId = userId, Status = MembershipStatus.Active },
        ...
        await hub.JoinSalon(orgId, conversationId);

        Assert.Contains($"salon:{conversationId}", groups.AddedGroups);
    }

    [Fact]
    public async Task JoinSalon_NonMember_Throws()
    {
        var userId = Guid.NewGuid();
        var orgId = Guid.NewGuid();
