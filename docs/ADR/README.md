...
| `ADR-026-tenant-account-awareness.md` | Tenant account awareness — the boutique's own Blossom balance and seat/customer allowances reach Aveline through a new internal snapshot endpoint that reuses the authoritative billing projections; the audience is **declared** by the API (`staff_query`) and required to be explicitly affirmative before anything is fetched, and a reply's numerals must all appear in the fetched snapshot |
| `ADR-027-run-lifecycle-telemetry.md` | Run lifecycle telemetry — the agent opens a `Running` row when a run starts and the completion report closes it, so `agent.runs_running` finally has a producer; a second stale-run arm reaps a row whose process died, and a cumulative `aveline.agent.runs_total` counter supplies the activity view the instantaneous gauges cannot |
| `ADR-028-pricing-questions-and-quotes.md` | Pricing questions, quotes and the discount ceiling — a price question that names a piece also dispatches the visual agent; commerce answers a discount question with no basket from the tier cap and the margin floor, and prices a *quote* (a new `purpose` on the order context) without ever pausing, settling or creating an order; ceilings are quoted in whole percent and rounded down |
| `ADR-029-payment-gateway-abstraction.md` | Payment gateway abstraction — a persisted provider-neutral payment intent, one `IPaymentProvider` SPI with a `Capabilities` record, money in integer minor units at the boundary, keyed-DI provider selection, a Development-gated mock, the operator route kept as the `manual` path, provider-first refunds and cancellations, the `Derived -> Verified` supersede path extracted into `IIncomeLedgerService.VerifyAsync`, and the unprocessed-event fail-safe for disputes |
