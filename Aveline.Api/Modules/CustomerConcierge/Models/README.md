...
- `CustomerPreference.cs` → `CustomerPreferences` table
- `CustomerEvent.cs` → `CustomerEvents` table
- `CustomerMemory.cs` → `CustomerMemory` table (has `float[]` vector column via pgvector)
- `CustomerInteraction.cs` → `CustomerInteractions` table
