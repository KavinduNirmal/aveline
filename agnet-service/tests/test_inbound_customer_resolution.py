...
def _resolver_returning(kind: str, **kwargs):
...
@pytest.mark.xfail(
    strict=True,
    reason="I1: a context-derived not_found currently vetoes every specialist (ADR-023).",
)
@pytest.mark.asyncio
...
@pytest.mark.xfail(
    strict=True,
    reason="I2: a not_found for an inbound sender currently asks for their own number (ADR-023).",
)
@pytest.mark.asyncio
async def test_inbound_sender_is_not_asked_for_their_own_phone_number(monkeypatch):
    """The rendered clarification must never ask for the number the sender already used.
